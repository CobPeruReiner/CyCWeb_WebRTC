...
export const TEjecForzadaPositivo = ({ visible, onHide, data, title, viewInicioEjecucion }) => {
    const IMAGE_URL = `${process.env.REACT_APP_ROUTE_IMAGE}`;

    const fechaTemplate = (rowData) => (rowData.SJFechaInicio ? new Date(rowData.SJFechaInicio).toLocaleDateString() : "-");

    const adjuntoTemplate = (rowData) =>
        rowData.SJAdjuntoInicio ? (
            <a href={`${IMAGE_URL}${rowData.SJAdjuntoInicio}`} target="_blank" rel="noopener noreferrer">
                Ver archivo
            </a>
        ) : (
            "-"
        ...
        <Dialog header={title} visible={visible} style={{ width: "800px" }} modal onHide={onHide} footer={<Button label="Cerrar" icon="pi pi-times" className="p-button-text" onClick={onHide} />}>
            <div className="p-card" style={{ background: "#e9ecef", padding: "1rem" }}>
                <DataTable value={data} paginator rows={5} emptyMessage="Sin registros">
                    <Column field="SJFechaInicio" header="FECHA" body={fechaTemplate} />
                    ...
                    <Column header="ACCIONES" body={(rowData) => <Button icon="pi pi-eye" className="p-button-text p-button-sm" tooltip="Ver detalle" onClick={() => viewInicioEjecucion(rowData)} />} />
                </DataTable>
            </div>
        </Dialog>
    );
